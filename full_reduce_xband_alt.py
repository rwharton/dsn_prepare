...
def trunc_chans_alt(infile):
    """
    Truncate infile to keep only the channels between 
    lofreq and hifreq specified in parameter file

    Alt version runs the script, instead of running 
    through module
    """
    tstart = time.time()
    ...
    # Truncate command
    trunc_cmd = "python " +\
                "%s/your_truncate2.py " %src_dir +\
                "-L %.4f " %(lofreq) +\
                "-H %.4f " %(hifreq) +\
                "-o %s%s " %(outdir, outbase) +\
                "%s " %infile
    
    print(trunc_cmd)  
    call(trunc_cmd, shell=True)
...
def convert_8bit_alt(infile):
    """
    Make an 8-bit version of the (presumed 32-bit float) 
    input data file, infile.  Output file will just be 
    [infile-base-name]-8bit.fil 

    Alt version run script in shell
    """
    tstart = time.time()
    inbase = get_inbase(infile)
    outbase = "%s-8bit" %(inbase)
    ...
    # 8-bit command
    bit_cmd = "python " +\
                "%s/your_8bit.py " %src_dir +\
                "-o %s " %outbase +\
                "%s " %infile

    # Run cmd
    print(bit_cmd)
    call(bit_cmd, shell=True)
...
def run_proc(indir, outdir, scanno):
    ...
    fil_file, trunc_time = trunc_chans_alt(fullband_file)
    ...
    # Convert to 8bit
    final_8bit, time_8bit = convert_8bit_alt(final_fil)

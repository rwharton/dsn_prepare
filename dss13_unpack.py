...
def unpack_rawfile(infile):
    ...
    # Make sure we have read in the right 
    # amount of data:
    # packets * (8 + 2 * 512 * 4)
    N_per_packet = par.PACKETS_PER_FILE * (8 + 2 * 512 * 4)
    if len(dat) != N_per_packet:
        print("Raw data file has unexpected size!")
        print("Expected size: %d samples" %(N_per_packet))
        print("Actual size:   %d samples" %(len(dat)))
        dat = np.zeros(N_per_packet, dtype='>H')
    else: pass
...
def check_input():
    ...
    if len(args) < 3:
        usage = "\ndss13_proc.py scan_num indir outdir [nproc]"
        print(usage)
        help_txt = "\n" +\
...
def convert_one_scan(scanno, indir, outdir_top, nproc):
    ...
    # Do the conversion
    tag = "%s-s%02d" %(name, scanno)
    outfile = raw2fil(indir, outdir, name, tag, date_str, pattern, nproc)
...
if __name__ == '__main__':
    if debug:
        ...
        with open('log%d.txt' %scanno, 'w') as sys.stdout:
            
            tstart = time.time() 

            name, outfile = convert_one_scan(scanno, indir, outdir_top, nproc)
            print(name)
            print(outfile)
